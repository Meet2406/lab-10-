...
def handle_poke_sel(event):
   ...
   if image_path:
      btn_set_desktop.state(['!DISABLED'])
      lbl_image.config('text'= None)
      photo.config('file' = image_path)
      
cbox_poke_sel.bind('<<ComboboxSelected>>', handle_poke_sel)

poke_ball_image = PhotoImage()
poke_label_image = ttk.Label(frm, image=poke_ball_image)
poke_label_image.grid(row=2, column=0, padx=10, pady=10,)
root.mainloop()